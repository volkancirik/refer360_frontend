...
PhotoSphereViewer.prototype._onMouseUp = function(evt) {
  this._stopMove(evt);
	...
	mh.innerText = mh.innerText + "|" + this.prop.position.longitude + "," + this.prop.position.latitude;
	var lm = document.getElementById("lastmouse");
	lm.innerText = parseFloat(lm.innerText) + 1;
...
PhotoSphereViewer.prototype._move = function(evt, log) {
    if(this.logmouse)
    {
	var event = new CustomEvent("mousechange", {"detail": this.prop.position.longitude+'|'+this.prop.position.latitude});
  ...
  if (this.prop.moving) {
    ...
    this.prop.mouse_y = y;
      ...
      // {
      // 	  var mh = document.getElementById("mousehistory");
      // 	   mh.innerText = mh.innerText + "|" + rotation.longitude + "," + rotation.latitude;
      // }
    if (log !== false) {
      this._logMouseMove(evt);
    }
